...
VS_CONTROL_POINT_OUTPUT WormVS( VS_CONTROL_POINT_INPUT Input )
{
...
HS_OUTPUT WormHS( InputPatch<VS_CONTROL_POINT_OUTPUT, INPUT_PATCH_SIZE> p, 
...
[domain("quad")]
DS_OUTPUT WormDS( HS_CONSTANT_DATA_OUTPUT input, 
...
float4 WormPS( DS_OUTPUT Input ) : SV_TARGET
{
    float3 N = normalize(Input.vNormal);
    float3 L = normalize(Input.vWorldPos - g_mcam_pos);
    return abs(dot(N, L)) * float4(0.3, 0.3,0.3, 1);
}
