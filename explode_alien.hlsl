...
cbuffer cbChangesEveryFrame : register( b0 )
{
    ...
    matrix View;
    matrix Projection;
    float4 time; //x = time, y = explode value for debug
};
...
//	for( int i=0; i<4; i++ )
//    {
//		float3 position = quad_pos[i]*Explode;
//		float4 PP=float4(position, 1.0);
//		PP.xyz=mul(PP.xyz, invView);
//		PP +=input[0].Pos;
//
//		output.Pos = mul(PP, View);
//		output.Pos = mul(output.Pos, Projection);
...
float4 PS( GSPS_INPUT input) : SV_Target
{
    ...
    float fLighting = saturate( dot( input.Norm, g_vLightDir ) );
