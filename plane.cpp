...
struct CB_PER_OBJECT
{
	...
	float		usecam;
	D3DXVECTOR4 displacement_lvl;
};
UINT                        planePerObjectBind = 0;

struct CB_PS_PER_FRAME
{
    D3DXVECTOR4 m_vLightDirAmbient;
};
UINT                        planePerFrameBind = 1;
...
const Points g_Quad[4] ={
	//bottom
	{1000.0f,0.0f,1000.0f,1,0},
	{1000.0f,0.0f,-1000.0f,0,0},
	{-1000.0f,0.0f,-1000.0f,0,1},
	{-1000.0f,0.0f,1000.0f,1,1},
};
...
E_PARTITION_MODE                    PartitionMode = PARTITION_INTEGER;


void plane::setdislvl(float input)
{
	displacmentlvl = input;
}
...
void plane::destroy()
{
	...
	SAFE_RELEASE(sandtex);
	SAFE_RELEASE(grasstex);

	SAFE_RELEASE( g_pRasterizerStateSolid);
	SAFE_RELEASE( g_pRasterizerStateWire);
...
HRESULT plane::CompileShaderFromFile( WCHAR* szFileName, D3D_SHADER_MACRO* pDefines, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
	  HRESULT hr = S_OK;
...
HRESULT plane::CompileShaderFromFile( WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
	 HRESULT hr = S_OK;
...
HRESULT plane::setup(ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc,void* pUserContext)
{
		HRESULT hr;
		useDT = true;
	showwire = false;
	displacmentlvl = 1.5f;
	tesslvl = 80;
	D3DXVECTOR3 vCenter( 100, 300, 200 );
	...
	V_RETURN( CompileShaderFromFile( L"tessplane.hlsl", NULL, "QuadTess_VS", "vs_5_0",  &pBlobVS ) );

    V_RETURN( CompileShaderFromFile( L"tessplane.hlsl", integerPartitioning, "QuadTess_HS", "hs_5_0", &pBlobHSInt ) );

	V_RETURN( CompileShaderFromFile( L"tessplane.hlsl", fracEvenPartitioning, "QuadTess_HS", "hs_5_0", &pBlobHSFracEven ) );

    V_RETURN( CompileShaderFromFile( L"tessplane.hlsl", fracOddPartitioning, "QuadTess_HS", "hs_5_0", &pBlobHSFracOdd ) );

    V_RETURN( CompileShaderFromFile( L"tessplane.hlsl", NULL, "QuadTess_DS", "ds_5_0", &pBlobDS ) );
    V_RETURN( CompileShaderFromFile( L"tessplane.hlsl", NULL, "QuadTess_PS", "ps_5_0", &pBlobPS ) );


	hr = D3DX11CreateShaderResourceViewFromFile( pd3dDevice, L"height.png", NULL, NULL, &g_pTextureRV, NULL );
	hr = D3DX11CreateShaderResourceViewFromFile( pd3dDevice, L"grass.jpg", NULL, NULL, &grasstex, NULL );
	hr = D3DX11CreateShaderResourceViewFromFile( pd3dDevice, L"sand.jpg", NULL, NULL, &sandtex, NULL );
...
void CALLBACK plane::Render( ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime, void* pUserContext, CFirstPersonCamera *g_Camera,D3DXVECTOR3 lightpos )
{
	HRESULT hr;
    ...
    pd3dImmediateContext->PSSetConstantBuffers( planePerFrameBind, 1, &g_pcbPSPerFrame );
	...
	pPerObject->displacement_lvl = D3DXVECTOR4( displacmentlvl, tesslvl, 0, 0 );
    pd3dImmediateContext->Unmap( g_pcbVSPerObject, 0 );

    pd3dImmediateContext->VSSetConstantBuffers( planePerObjectBind, 1, &g_pcbVSPerObject );
	pd3dImmediateContext->VSSetConstantBuffers( planePerObjectBind, 1, &g_pcbVSPerObject );
    pd3dImmediateContext->HSSetConstantBuffers( planePerObjectBind, 1, &g_pcbVSPerObject );
    pd3dImmediateContext->DSSetConstantBuffers( planePerObjectBind, 1, &g_pcbVSPerObject );
	pd3dImmediateContext->OMSetBlendState(NULL,NULL,0xffffffff);
    pd3dImmediateContext->PSSetConstantBuffers( planePerObjectBind, 1, &g_pcbVSPerObject );
	...
	pd3dImmediateContext->PSSetShaderResources( 0, 1, &g_pTextureRV );
	pd3dImmediateContext->PSSetShaderResources( 1, 1, &grasstex );
	pd3dImmediateContext->PSSetShaderResources( 2, 1, &sandtex );
	...
	pd3dImmediateContext->GSSetShader(NULL,NULL,0);
	  if (PartitionMode == PARTITION_INTEGER)
        pd3dImmediateContext->HSSetShader( g_pHullShaderInteger, NULL, 0 );
    else if (PartitionMode == PARTITION_FRACTIONAL_EVEN)
        pd3dImmediateContext->HSSetShader( g_pHullShaderFracEven, NULL, 0 );
    else if (PartitionMode == PARTITION_FRACTIONAL_ODD)
...
void plane::toggleWire(bool input)
{
	showwire = input;
}

void plane::toggleDT(bool input)
{
	useDT = input;
}

void plane::settessmethod(int input)
{
	if(input == 0)
	{
		PartitionMode = PARTITION_INTEGER;
	}
	else if(input == 1)
	{
		PartitionMode = PARTITION_FRACTIONAL_EVEN;
	}
	else
	{
		PartitionMode = PARTITION_FRACTIONAL_ODD;
	}
}

void plane::setTesslvl(float input)
{
	tesslvl = input;
}