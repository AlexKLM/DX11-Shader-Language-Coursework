...
HRESULT billboard::CompileShaderFromFile( WCHAR* szFileName, D3D_SHADER_MACRO* pDefines, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
	  HRESULT hr = S_OK;
...
HRESULT billboard::setup(ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc,void* pUserContext,D3DXVECTOR3 pos)
{
	type = 0;
		HRESULT hr;
	//300,-200,-200
  ...
  /*  D3DXMATRIXA16 m;
    ...
    V_RETURN( CompileShaderFromFile( L"raytrace1.hlsl", "VS_CanvasSetup", "vs_4_0", &pVertexShaderBuffer ) );

    ID3DBlob* pPixelShaderBuffer = NULL;
    V_RETURN( CompileShaderFromFile( L"raytrace1.hlsl", "Image", "ps_4_0", &pPixelShaderBuffer ) );
...
void billboard::setup_texture()
{
	
}

void billboard::change_screen()
{
	if(type == 0)
	{
		type = 1;
...
void CALLBACK billboard::Render( ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime, void* pUserContext, CFirstPersonCamera *g_Camera,D3DXVECTOR3 lightpos )
{
	HRESULT hr;
...
void CALLBACK billboard::RenderTexture( ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime, void* pUserContext,CDXUTDirectionWidget *g_LightControl )
{
	HRESULT hr;
    ...
    mView = *billboard_Camera.GetViewMatrix();
	...
	pPerObject->viewpos = D3DXVECTOR4(Eye.x,Eye.y,Eye.z,1);
